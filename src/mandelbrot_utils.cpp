...
int update_rule_case_1(MyComplex & z_0, MyComplex & c_0, const double x_0, const double y_0, const double delta_x, const double delta_y, const int i, const int j, double R_c, int maxIterations)
{
	...
	step_result = calcNumConvergence(z_0, c_0, 2, R_c, maxIterations); // 2 is for the exponent z_i^2
...
void calculate_convergence_radius(std::ofstream & out_stream, const run_config & run_configuration)
{
	...
	for (int i = 0; i <= run_configuration.N_xmax; i++){
		for (int j = 0; j <= run_configuration.N_ymax; j++){
			...
			n_iter_temp = update_formula(z_0, c_0, run_configuration.x_0, run_configuration.y_0, delta_x, delta_y, i, j, run_configuration.R_c, run_configuration.maxIterations);
