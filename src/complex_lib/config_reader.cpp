...
ConfigurationLoader::ConfigurationLoader(const std::string &path_to_config) {
  YAML::Node config_reader = YAML::LoadFile(path_to_config);

  this->calc_configuration_.calculation_case = config_reader["case"].as<int>();
  this->calc_configuration_.x_lower_bound =
      config_reader["x_lower"].as<double>();
  this->calc_configuration_.y_lower_bound =
      config_reader["y_lower"].as<double>();
  this->calc_configuration_.x_upper_bound =
      config_reader["x_upper"].as<double>();
  this->calc_configuration_.y_upper_bound =
      config_reader["y_upper"].as<double>();
  ...
  this->calc_configuration_.name_result =
      config_reader["name_result"].as<std::string>();
  this->calc_configuration_.max_iterations =
      config_reader["max_iter"].as<int>();

  this->path_ = path_to_config;
}

const run_config ConfigurationLoader::get_run_config() const {
